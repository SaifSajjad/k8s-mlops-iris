...
- Jenkins (updated 2026-06-09): `jenkins/Jenkinsfile` is a 15-stage Windows PowerShell declarative pipeline with `agent { label 'windows' }`. Fixes: Unix `sh`/`eval`/`sleep` replaced; MLflow URI set to port 5002; stage 2 checks `py` launcher not `.venv`; stage 3 adds idempotent `minikube start/update-context`; stage 6 adds HTTP readiness loop (Invoke-WebRequest HTTP 200); stage 7 auto-creates venv via `py -3.11 -m venv .venv` for fresh SCM workspaces; `post.always` uses `returnStatus: true` (non-blocking); README corrected to document native Windows Jenkins as recommended and Linux Docker controller alone as insufficient. `docs/JENKINS_PIPELINE_VERIFICATION.md` updated. Jenkins not installed locally; live run pending explicit user approval.
